...
export function KpiCard({
  ...
  return (
    <div
      className="flex min-h-[7rem] flex-col gap-1 rounded-card border border-subtle bg-surface p-4"
    ...
    >
      ...
      <div className="flex items-baseline gap-1">
        <span className={`display tabular text-kpi leading-none font-bold ${valueTone ? VALUE_COLOR[valueTone] : 'text-primary'}`}>
          {value}
        </span>
        {unit && <span className="text-label text-muted">{unit}</span>}
      </div>
      {sub && <span className="truncate text-label text-secondary">{sub}</span>}
      {note && <span className="truncate text-label text-muted">{note}</span>}
    </div>
  )
}

/**
 * Responsive KPI strip: auto-packs as many compact cards as fit — ~8 on wide
 * desktop, ~4 on tablet, 2 on mobile — without per-breakpoint column juggling.
 */
